...
type PortalRole = 'admin' | 'cfi' | 'senior_instructor' | 'instructor' | 'pilot' | 'student';
type AcceptanceUser = {
  id: string;
  role: PortalRole;
  email: string;
...
const credentials = JSON.parse(process.env.ACCEPTANCE_USERS_JSON || '{}') as Record<string, AcceptanceUser>;
const roles: PortalRole[] = ['admin', 'cfi', 'senior_instructor', 'instructor', 'pilot', 'student'];
const staffRoles = new Set<PortalRole>(['admin', 'cfi', 'senior_instructor', 'instructor']);
...
for (const role of roles) {
  test(`${role} can authenticate and access the correct mobile portal navigation`, async ({ page }, testInfo) => {
    const credentialKey = `${testInfo.project.name}:${role}`;
    const user = credentials[credentialKey];
    expect(user, `Missing acceptance credentials for ${credentialKey}`).toBeTruthy();
    ...
    await page.evaluate((userId) => {
      sessionStorage.setItem(`safety-login-warning-dismissed:${userId}`, 'true');
    }, user.id);
    ...
    if (staffRoles.has(role)) {
      await expect(page.getByRole('heading', { name: 'Protect your staff account' })).toBeVisible();
      ...
      await expect(page.getByRole('status').filter({ hasText: 'Authenticator protection is now active' })).toBeHidden({
        timeout: 8_000,
      });
    }

    await expect(page.getByRole('button', { name: 'Logout' })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Bendigo Flying Club' })).toBeVisible();
    const profileReminder = page.getByRole('button', { name: 'Later', exact: true });
    ...
    for (const item of expectedMenuItems[role]) {
      await expect(mobileMenu.getByRole('button', { name: item, exact: true })).toBeVisible();
    }
    if (role !== 'admin') {
      await expect(mobileMenu.getByRole('button', { name: 'Financial Dashboard', exact: true })).toHaveCount(0);
    }
    await mobileMenu.getByRole('button', { name: 'Close navigation menu' }).evaluate((button) => {
