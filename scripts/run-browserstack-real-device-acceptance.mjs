...
const mfaCodeLocator = By.css(
  'input[aria-label="Six-digit authenticator code"]',
);
      ...
      new Promise((_, reject) => {
        timeoutId = setTimeout(
          () =>
            reject(
              new Error(
                `${description} timed out after ${timeoutMs / 1_000} seconds.`,
              ),
            ),
          timeoutMs,
        );
...
for (const device of devices) {
  for (const role of roles) {
    if (!credentials[`${device.key}:${role}`]) {
      throw new Error(
        `Missing disposable acceptance credentials for ${device.key}:${role}.`,
      );
    }
  }
}
...
const stopRemoteSession = async (sessionId) => {
  if (!sessionId) return;
  const authorization = Buffer.from(`${username}:${accessKey}`).toString(
    "base64",
  );
  try {
...
const findVisible = async (driver, locator, timeout = 20_000) => {
  return driver.wait(
    ...
    timeout,
    `No visible element found for ${locator}.`,
  );
};
...
const reportBrowserState = async (driver, deviceName) => {
  try {
    ...
    console.error(
      `${deviceName} browser state: URL=${url}; title=${title}; text=${summary}`,
    );
  } catch (error) {
...
const testRole = async (driver, deviceKey, role) => {
  ...
  if (
    enteredEmail !== credential.email ||
    enteredPassword !== credential.password
  ) {
    throw new Error(
      `${deviceKey}:${role} credentials were not entered exactly on the mobile keyboard.`,
    );
  }
  ...
  if (staffRoles.has(role)) {
    await findVisible(
      driver,
      By.xpath(
        '//*[self::h1 or self::h2][normalize-space(.)="Protect your staff account"]',
      ),
      60_000,
    );
    ...
    await reveal.click();
    const secret = await (
      await findVisible(driver, By.css("details code"))
    ).getText();
    const submitEnrollmentCode = async () => {
      ...
      if ((await codeInput.getAttribute("value")) !== code) {
        throw new Error(
          `${deviceKey}:${role} MFA code was not entered exactly on the mobile keyboard.`,
        );
      }
    ...
    if (!authenticated) {
      throw new Error(
        `${deviceKey}:${role} did not leave MFA enrollment after two fresh codes.`,
      );
    }
  }
  ...
  await findVisible(
    driver,
    By.xpath(
      '//*[self::h1 or self::h2][normalize-space(.)="Bendigo Flying Club"]',
    ),
  );

  const later = await optionalVisible(
    driver,
    By.xpath('//button[normalize-space(.)="Later"]'),
  );
  if (later) await later.click();

  const openMenu = await findVisible(
    driver,
  ...
  for (const item of expectedMenuItems[role]) {
    const menuItem = await sidebar.findElement(
      By.xpath(`.//button[normalize-space(.)="${item}"]`),
    );
  ...
  if (role !== "admin" || !expectFinancialDashboard) {
    const forbidden = await sidebar.findElements(
      By.xpath('.//button[normalize-space(.)="Financial Dashboard"]'),
    );
    if (forbidden.length > 0)
      throw new Error(`${role} can see the admin Financial Dashboard.`);
  }
  const closeMenu = await sidebar.findElement(
    By.css('button[aria-label="Close navigation menu"]'),
  );
  await closeMenu.click();

  const calendar = await findVisible(
    driver,
    By.css('header button[aria-label="Calendar"]'),
  );
  await calendar.click();
  await driver.wait(
    async () => (await driver.getCurrentUrl()).endsWith("/calendar"),
    20_000,
  );
  await findVisible(driver, logoutLocator);

  const profile = await findVisible(
    driver,
    By.css('header button[aria-label="Profile"]'),
  );
  await profile.click();
  await driver.wait(
    async () => new URL(await driver.getCurrentUrl()).pathname === "/",
    20_000,
  );
  await findVisible(driver, logoutLocator);
  ...
  if (expectedMenuItems[role].includes("Maintenance")) {
    ...
    await findVisible(
      driver,
      By.xpath(
        '//*[self::h1 or self::h2][normalize-space(.)="Maintenance Board"]',
      ),
    );
    await findVisible(
      driver,
      By.xpath('//button[normalize-space(.)="Report Defect"]'),
    );
    ...
    await findVisible(
      driver,
      By.xpath('//button[normalize-space(.)="Deferred"]'),
    );
  }
  ...
  if (hasHorizontalOverflow)
    throw new Error(`${role} portal has horizontal overflow.`);
};
  ...
  for (const device of devices) {
    ...
    try {
      ...
      await setSessionStatus(
        driver,
        "passed",
        "All six authenticated role journeys passed.",
      );
    } catch (error) {
      failed = true;
      if (driver)
