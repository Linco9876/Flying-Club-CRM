...
export const FlightReviewsTab: React.FC<FlightReviewsTabProps> = ({
  studentId,
  studentName,
}) => {
  const { user } = useAuth();
  ...
  const eligibleReviewers = useMemo(() => {
    ...
    const allowedRoles = new Set(
      selectedTemplate.configuration.allowed_reviewer_roles,
    );
    return users
  ...
  const selectedRecord = reviews.records.find(
  ...
  const beginReview = async () => {
    if (!startForm.templateId) {
      toast.error("Select a review or test form");
      return;
    }
    ...
    setStarting(true);
    try {
      const record = await reviews.startReview({
  ...
  const acknowledge = async (recordId: string) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to acknowledge review",
      );
    } finally {
        ...
        <header className="flex items-start justify-between gap-3 bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 px-5 py-5 text-white">
          <div>
            <p className="text-xs font-bold uppercase text-blue-200">
              New Pilot File record
            </p>
            <h2 className="mt-1 text-xl font-bold">
              Start a review or test for {studentName}
            </h2>
          </div>
          <button
            type="button"
          ...
          <label className="text-sm font-medium text-gray-700 dark:text-gray-200">
            ...
            <select
              ...
              onChange={(event) =>
                setStartForm((current) => ({
                  ...current,
                  examinerMode: event.target.value as "internal" | "external",
                }))
              }
              className={inputClass}
            >
  ...
  if (reviews.loading) {
    return (
      <PortalSectionLoader
        message="Loading reviews and tests"
        detail="Preparing outcomes, evidence and currency information..."
      />
    );
  }
      ...
      <div className="space-y-4">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-blue-100">
                Review outcomes, assessment evidence and acknowledgements for{" "}
                {studentName}.
              </p>
            </div>
            {isStaff && (
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-10 text-center shadow-sm dark:border-[#2c3440] dark:bg-[#171a21]">
          ...
          <h2 className="mt-3 text-lg font-bold text-gray-950 dark:text-gray-100">
            No reviews or tests recorded
          </h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Completed forms will remain here as part of this member's Pilot
            File.
          </p>
        </div>
    ...
    <div className="space-y-4">
      ...
      {sortedRecords.map((record) => {
        ...
        const reviewer =
          ...
          users.find((item) => item.id === record.reviewerUserId)?.name ||
        ...
        const acknowledgementRequired = Boolean(
          record.templateSnapshot.review_configuration?.candidate_ack_required,
        );
        return (
          <article
            key={record.id}
            className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-[#2c3440] dark:bg-[#171a21]"
          >
            <button
              type="button"
            ...
            {expanded && (
              <div className="space-y-5 border-t border-gray-200 p-4 dark:border-[#2c3440] sm:p-5">
                  ...
                  <section className="grid gap-3 lg:grid-cols-3">
                    ...
                    {record.remedialPlan && (
                      <div className="rounded-lg bg-amber-50 p-3 dark:bg-amber-500/10">
                        <h4 className="text-xs font-bold uppercase text-amber-800 dark:text-amber-200">
                ...
                <section>
                  <h4 className="font-bold text-gray-950 dark:text-gray-100">
                  ...
                  <div className="mt-3 grid gap-2 lg:grid-cols-2">
                    {items.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-lg border border-gray-200 p-3 dark:border-[#343b46]"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-sm font-semibold text-gray-950 dark:text-gray-100">
                              {item.title}
                            </p>
                          </div>
                          <span
                            className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${item.result === "satisfactory" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-500/15 dark:text-emerald-200" : item.result === "further_training" ? "bg-amber-100 text-amber-900 dark:bg-amber-500/15 dark:text-amber-200" : "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-100"}`}
                          >
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {attachments.map((attachment) => (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            void openAttachment(attachment.filePath)
                          }
                          className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-[#39414d] dark:text-gray-200 dark:hover:bg-[#11141a]"
                        >
                ...
                <div className="flex flex-col gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-500/30 dark:bg-blue-500/10 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {record.candidateAckAt && (
                        <p className="mt-1 text-xs text-gray-600 dark:text-gray-300">
                          {record.candidateAckName} |{" "}
                          {format(
                            parseISO(record.candidateAckAt),
                            "d MMM yyyy, h:mm a",
                          )}
                        </p>
                      )}
                  ...
                  {isCandidate &&
                    ...
                    acknowledgementRequired && (
                      <button
                        type="button"
                        onClick={() => void acknowledge(record.id)}
                        disabled={acknowledgingId === record.id}
                        className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-60"
                      >
                ...
                {isStaff && record.status !== "cancelled" && (
                  <div className="flex justify-end border-t border-gray-200 pt-4 dark:border-[#2c3440]">
                    <button
                      ...
                      className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-500"
                    >
      ...
      {selectedRecord && user && (
        <FlightReviewRecordEditor
          ...
          onCreateAttachmentUrl={reviews.createAttachmentUrl}
        />
      )}
    </div>
  );
