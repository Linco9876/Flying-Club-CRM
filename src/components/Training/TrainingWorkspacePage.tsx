...
const TrainingCourseCatalog = lazy(() =>
  import("./TrainingCourseCatalog").then((module) => ({
    default: module.TrainingCourseCatalog,
  })),
);
const FlightReviewWorkspace = lazy(() =>
  import("./FlightReviewWorkspace").then((module) => ({
    default: module.FlightReviewWorkspace,
  })),
);
const InstructorReviewTemplateWorkspace = lazy(() =>
  import("./InstructorReviewTemplateWorkspace").then((module) => ({
    default: module.InstructorReviewTemplateWorkspace,
  })),
);
...
export const TrainingWorkspacePage: React.FC = () => {
  ...
  const activeWorkspace: TrainingWorkspace = useMemo(() => {
    ...
    if (
      requestedWorkspace === "instructor-standards" &&
  ...
  const tabs = [
    ...
    {
      id: "reviews" as const,
      label: "Flight Reviews & Tests",
      icon: ClipboardCheck,
    },
    ...(canSeeInstructorStandards
      ? [
          {
            id: "instructor-standards" as const,
  ...
  return (
    <div className="space-y-5">
      ...
      <Suspense
        fallback={
          <PortalSectionLoader message="Preparing training workspace" />
        }
      >
        {activeWorkspace === "courses" && <TrainingCourseCatalog />}
        {activeWorkspace === "reviews" && <FlightReviewWorkspace />}
        {activeWorkspace === "instructor-standards" &&
          canSeeInstructorStandards && <InstructorReviewTemplateWorkspace />}
      </Suspense>
    </div>
  );
};
