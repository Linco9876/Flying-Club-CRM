...
const resultOptions: Array<{
...
}> = [
  {
    ...
    label: "Not assessed",
    className:
      "border-gray-300 text-gray-600 dark:border-[#3b414c] dark:text-gray-300",
  },
  {
    value: "satisfactory",
    label: "Satisfactory",
...
> = ({ flightLog, candidate, examiner, onClose, onCompleted }) => {
  ...
  const instructorLevel: InstructorComplianceLevel = isSenior
    ? "senior_instructor"
    : "instructor";
  ...
  const [flightMinutes, setFlightMinutes] = useState(() =>
    Math.max(
      0,
      Math.round(
        ((flightLog.dual_time || 0) + (flightLog.solo_time || 0)) * 60,
      ),
    ),
  );
  ...
  const handleSubmit = async () => {
    ...
    if (!medicalSighted || !emergencyControlPlanConfirmed) {
      toast.error(
        "Confirm the medical and emergency control plan before completing the check",
      );
      return;
    }
    if (requiredIncomplete.length > 0) {
      toast.error(
        `Complete all required checklist items (${requiredIncomplete.length} remaining)`,
      );
      return;
    }
    ...
    if (unsatisfactoryCount > 0 && !developmentPlan.trim()) {
      toast.error(
        "Add a remedial development plan for the below-standard items",
      );
      return;
    }
      ...
      <header className="bg-gradient-to-r from-slate-950 via-cyan-950 to-slate-900 px-4 py-5 text-white sm:px-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            aria-label="Close instructor check"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="space-y-6 p-4 sm:p-6">
            ...
            <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100">
              ...
              <div className="mt-2 flex items-center gap-2">
                <input
                  ...
                  value={groundMinutes}
                  onChange={(event) =>
                    setGroundMinutes(Number(event.target.value))
                  }
                  className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal dark:border-[#3b414c] dark:bg-[#171a21]"
                />
            ...
            <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100">
              ...
              <div className="mt-2 flex items-center gap-2">
                <input
                  ...
                  value={flightMinutes}
                  onChange={(event) =>
                    setFlightMinutes(Number(event.target.value))
                  }
                  className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal dark:border-[#3b414c] dark:bg-[#171a21]"
                />
                <span className="text-xs text-gray-500">min</span>
          ...
          <section className="rounded-lg border border-cyan-200 bg-cyan-50 p-4 text-sm text-cyan-950 dark:border-cyan-400/20 dark:bg-cyan-950/20 dark:text-cyan-100">
            ...
            <button
              ...
              className="mt-3 inline-flex items-center gap-1 font-semibold text-cyan-800 dark:text-cyan-200"
            >
              {showTolerances ? (
              ...
              RAAP 7 flight tolerances
            </button>
            {showTolerances && (
              <div className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-3">
                <span>Taxi centreline: +/-1 m</span>
                <span>Heading: +/-10 degrees</span>
                <span>Level flight: +/-150 ft, +/-10 kt</span>
                <span>Climb: -0/+5 kt</span>
                <span>Turns: +/-5 degrees bank</span>
                <span>Steep turn: +/-150 ft, +/-10 degrees</span>
                <span>Final: -0/+5 kt</span>
                <span>Touchdown: +/-60 m</span>
                <span>Landing centreline: +/-2 m</span>
              </div>
            )}
          </section>
          ...
          <section className="grid gap-4 md:grid-cols-2">
            <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100 md:col-span-2">
              ...
              <input
                value={briefingLesson}
                onChange={(event) => setBriefingLesson(event.target.value)}
                placeholder="For example: Forced landings"
                className="mt-2 w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal dark:border-[#3b414c] dark:bg-[#11141a]"
              />
            </label>
            <label className="flex min-h-12 items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-800 dark:border-[#343943] dark:text-gray-100">
              <input
                type="checkbox"
                checked={medicalSighted}
                onChange={(event) => setMedicalSighted(event.target.checked)}
            ...
            <label className="flex min-h-12 items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-800 dark:border-[#343943] dark:text-gray-100">
              <input
                type="checkbox"
                checked={emergencyControlPlanConfirmed}
                onChange={(event) =>
                  setEmergencyControlPlanConfirmed(event.target.checked)
                }
                className="h-5 w-5 rounded border-gray-300 text-cyan-600"
              />
          ...
          <section>
            ...
            <div className="space-y-5">
              {sections.map(([section, sectionItems]) => (
                <div
                  key={section}
                  className="overflow-hidden rounded-lg border border-gray-200 dark:border-[#343943]"
                >
                  <div className="border-b border-gray-200 bg-slate-900 px-4 py-3 text-sm font-bold text-white dark:border-[#343943]">
                    {section}
                  </div>
                        ...
                        <article key={item.id} className="p-4">
                          <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                            <div className="min-w-0">
                              <div className="flex items-start gap-2">
                                <span className="mt-0.5 rounded bg-gray-100 px-1.5 py-0.5 text-[11px] font-bold text-gray-600 dark:bg-[#262b33] dark:text-gray-300">
                                  {item.code}
                                </span>
                            ...
                            <div className="grid shrink-0 grid-cols-3 gap-2">
                              {resultOptions.map((option) => (
                                <button
                                  ...
                                  type="button"
                                  onClick={() =>
                                    setItemResult(item.id, option.value)
                                  }
                                  className={`min-h-10 rounded-md border px-2 py-2 text-xs font-bold transition ${option.className} ${result === option.value ? "ring-2 ring-cyan-500 ring-offset-1 dark:ring-offset-[#171a21]" : "opacity-65 hover:opacity-100"}`}
                                >
                                  {option.value === "satisfactory" && (
                                    <Check className="mx-auto mb-0.5 h-3.5 w-3.5" />
                                  )}
                                  {option.value === "unsatisfactory" && (
                                    <XCircle className="mx-auto mb-0.5 h-3.5 w-3.5" />
                                  )}
                                  {option.label}
                                </button>
                              ))}
                          ...
                          {result === "unsatisfactory" && (
                            <textarea
                              ...
                              placeholder="Record the observed deficiency and evidence..."
                              rows={2}
                              className="mt-3 w-full rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-950 dark:border-red-400/20 dark:bg-red-950/20 dark:text-red-100"
                            />
                          )}
                        </article>
                      );
          ...
          <section className="grid gap-4 md:grid-cols-2">
            ...
            <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100">
              ...
              <textarea
                value={deficiencies}
                onChange={(event) => setDeficiencies(event.target.value)}
                rows={3}
                className="mt-2 w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-normal dark:border-[#3b414c] dark:bg-[#11141a]"
              />
            </label>
            <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100">
              Development / remedial plan
              <textarea
          ...
          {checkType === "renewal" && (
            <section className="rounded-lg border border-amber-300 bg-amber-50 p-4 dark:border-amber-400/30 dark:bg-amber-950/20">
              <label className="block text-sm font-bold text-amber-950 dark:text-amber-100">
                RAAus instructor renewal form{" "}
                <span className="text-red-600">*</span>
                <span className="mt-1 block text-xs font-normal leading-5 text-amber-800 dark:text-amber-200">
                  Attach the completed INS002 or the current replacement form.
                  The renewal cannot be completed without it.
                </span>
                <span className="mt-3 flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-md border-2 border-dashed border-amber-400 bg-white px-4 py-3 dark:bg-[#11141a]">
                  ...
                  {renewalForm?.name || "Choose renewal form"}
                  <input
                    type="file"
                    className="sr-only"
                    onChange={(event) =>
                      setRenewalForm(event.target.files?.[0] || null)
                    }
                  />
                </span>
              </label>
          ...
          <section
            className={`rounded-lg border p-4 ${unsatisfactoryCount > 0 ? "border-red-200 bg-red-50 dark:border-red-400/20 dark:bg-red-950/20" : "border-emerald-200 bg-emerald-50 dark:border-emerald-400/20 dark:bg-emerald-950/20"}`}
          >
          ...
          <footer className="flex flex-col-reverse gap-3 border-t border-gray-200 pt-5 dark:border-[#343943] sm:flex-row sm:justify-end">
            <button
              type="button"
              onClick={onClose}
              className="min-h-11 rounded-md border border-gray-300 px-5 py-2 text-sm font-semibold text-gray-700 dark:border-[#3b414c] dark:text-gray-200"
            >
              Cancel
            </button>
            <button
              type="button"
              disabled={submitting || requiredIncomplete.length > 0}
              onClick={handleSubmit}
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-cyan-700 px-5 py-2 text-sm font-bold text-white transition hover:bg-cyan-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {submitting ? (
