                ...
                <div className="mt-3 space-y-3">
                  {form.breaks.map((item, index) => (
                    <div key={index} className="rounded-lg bg-gray-50 p-3">
                      <div className="grid gap-2 sm:grid-cols-3"><input type="datetime-local" value={item.breakStart} onChange={event => setForm({ ...form, breaks: form.breaks.map((value, itemIndex) => itemIndex === index ? { ...value, breakStart: event.target.value } : value) })} className="rounded-md border border-gray-300 px-2 py-2 text-sm" /><input type="datetime-local" value={item.breakEnd} onChange={event => setForm({ ...form, breaks: form.breaks.map((value, itemIndex) => itemIndex === index ? { ...value, breakEnd: event.target.value } : value) })} className="rounded-md border border-gray-300 px-2 py-2 text-sm" /><select value={item.breakType} onChange={event => setForm({ ...form, breaks: form.breaks.map((value, itemIndex) => itemIndex === index ? { ...value, breakType: event.target.value as BreakDraft['breakType'] } : value) })} className="rounded-md border border-gray-300 px-2 py-2 text-sm"><option value="break">Break</option><option value="rest">Rest</option><option value="split_duty_rest">Split-duty rest</option></select></div>
