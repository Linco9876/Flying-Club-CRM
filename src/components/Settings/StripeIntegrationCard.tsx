...
const setupSteps = [
  'Connect the club Stripe account',
  'Enable trial-flight voucher checkout',
  'Prepare member account top-ups and flight payments',
  'Keep Stripe secrets server-side in Supabase',
];
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-blue-100">
              Link Stripe once, then use it for online trial-flight vouchers, member account top-ups, and card payments for logged flights as those payment screens are enabled.
            </p>
          </div>
          <span className={`inline-flex w-fit rounded-full px-3 py-1 text-xs font-semibold ${connected ? 'bg-green-400/15 text-green-100 ring-1 ring-green-300/30' : 'bg-amber-400/15 text-amber-100 ring-1 ring-amber-300/30'}`}>
            {connected ? 'Connected' : configured ? 'Ready to connect' : 'Needs Stripe keys'}
          </span>
        </div>
      </div>
      ...
      <div className="p-5 space-y-5">
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
            ...
            <p className={`mt-1 text-sm font-semibold ${connected ? 'text-green-700' : 'text-amber-700'}`}>
              {connected ? 'Stripe account linked' : 'Not connected'}
            </p>
          </div>
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
            ...
            <p className="mt-1 text-sm font-medium text-gray-800">
              {connected ? (stripeStatus?.livemode ? 'Live payments' : 'Test mode') : configured ? 'OAuth ready' : 'Setup required'}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="rounded-lg border border-gray-200 p-4">
            <p className="text-sm font-semibold text-gray-900">What the setup enables</p>
          ...
          <div className="rounded-lg border border-gray-200 p-4">
            <p className="text-sm font-semibold text-gray-900">Stripe redirect URI</p>
            <p className="mt-1 text-xs text-gray-500">This must be allowed in Stripe Connect OAuth settings before the one-click link can complete.</p>
        ...
        {stripeStatus && !configured && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            <p className="font-semibold">Stripe needs one secure setup step first.</p>
            <p className="mt-1">
              Add <span className="font-mono">STRIPE_SECRET_KEY</span> and <span className="font-mono">STRIPE_CONNECT_CLIENT_ID</span> to Supabase Edge Function secrets. Stripe does not let the CRM read or create those account-wide secrets through OAuth.
            </p>
          </div>
        )}
