...
export type InstructorComplianceCheckType =
  "initial_issue" | "sp_check" | "renewal";
export type InstructorComplianceLevel = "instructor" | "senior_instructor";
export type InstructorComplianceOutcome =
  "not_assessed" | "satisfactory" | "unsatisfactory";
export type InstructorComplianceItemResult =
  "not_assessed" | "satisfactory" | "unsatisfactory";
...
export interface SaveInstructorComplianceTemplate {
  id?: string;
  name: string;
  description: string;
  version: string;
...
const mapItem = (value: unknown): InstructorComplianceCourseItem => {
  ...
  return {
    ...
    applicableLevels: Array.isArray(row.applicable_levels)
      ? row.applicable_levels
      : [],
    applicableCheckTypes: Array.isArray(row.applicable_check_types)
      ? row.applicable_check_types
      : [],
...
export function useInstructorCompliance(
  options: boolean | { enabled?: boolean; includeRecords?: boolean } = true,
) {
  const enabled =
    typeof options === "boolean" ? options : (options.enabled ?? true);
  const includeRecords =
    typeof options === "boolean" ? true : (options.includeRecords ?? true);
  ...
  const fetchCompliance = useCallback(async () => {
    ...
    try {
      ...
      const [courseResult, itemResult, recordResult] = includeRecords
        ? await Promise.all([...baseQueries, recordQuery])
        : [...(await Promise.all(baseQueries)), null];
      ...
      if (includeRecords) {
        if (!recordResult) throw new Error("Instructor records did not load");
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to load instructor compliance records";
      console.error("Error loading instructor compliance:", err);
      setError(message);
    } finally {
      setLoading(false);
    }
  }, [enabled, includeRecords]);
  ...
  const saveRecord = useCallback(
    async (input: SaveInstructorComplianceRecord) => {
      const row = {
  ...
  const uploadRenewalForm = useCallback(
  ...
  const saveTemplate = useCallback(
    async (input: SaveInstructorComplianceTemplate) => {
      const { data, error: saveError } = await supabase.rpc(
        "save_instructor_compliance_template",
        {
          p_course_id: input.id || null,
          p_name: input.name,
          ...
          p_items: input.items.map((item) => ({
            ...
            applicable_levels: item.applicableLevels,
            applicable_check_types: item.applicableCheckTypes,
          })),
        },
      );
      if (saveError) throw saveError;
