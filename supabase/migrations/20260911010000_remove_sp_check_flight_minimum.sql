-- Remove the flight-duration minimum only for S&P checks. Preserve all other completion and currency rules.
...
BEGIN
  ...
  IF NEW.status IN ('completed', 'remedial_required') THEN
    ...
    END IF;
    IF NEW.check_type <> 'sp_check' AND NEW.outcome = 'satisfactory' AND NEW.flight_minutes < 60 THEN
      RAISE EXCEPTION 'A satisfactory instructor renewal or initial issue must record at least 60 minutes in flight';
    END IF;
...
COMMENT ON FUNCTION public.prepare_instructor_compliance_record() IS
  'Validates instructor compliance records; S&P checks have no minimum flight duration. Renewals and initial issue retain the 60-minute satisfactory-flight minimum.';
