-- Allow a CFI / DCFI to move an existing supervision allocation from the
-- calendar. The operation remains an exact-slot commitment and deliberately
-- bypasses roster availability, while retaining authorisation, qualification,
-- location/activity scope, duty and concurrent-capacity validation.
...
declare
  ...
  v_previous_supervisor_id uuid;
  v_previous_supervision_status text;
...
begin
  ...
  if not v_booking.supervision_required
    or v_booking.supervision_status not in ('pending', 'assigned', 'acknowledged')
  then
  ...
  v_previous_supervisor_id := v_booking.supervising_instructor_id;
  v_previous_supervision_status := v_booking.supervision_status;

  if v_previous_supervisor_id = p_supervisor_id then
    ...
    return jsonb_build_object(
      ...
      'supervisionStatus', v_booking.supervision_status,
  ...
  if found then
    v_preflight_minutes := greatest(0, coalesce(v_requirement.preflight_minutes, 30));
    v_postflight_minutes := greatest(0, coalesce(v_requirement.postflight_minutes, 30));
  end if;
  ...
  set status = 'invalidated',
      ...
      ended_by = v_actor_id,
      end_reason = case
        when v_previous_supervisor_id is null then 'Replaced by a CFI supervisor allocation'
        else 'Reassigned to another supervisor by a CFI / DCFI'
      end,
  ...
  insert into public.booking_supervision_commitments (
    ...
    jsonb_build_object(
      ...
      'source', 'calendar_supervision_layer',
      'allocationMode', case
        when v_previous_supervisor_id is null then 'cfi_assigned'
        else 'cfi_reassigned'
      end,
      'assignedByCfi', v_actor_id,
      'previousSupervisorId', v_previous_supervisor_id,
      'rosterAvailabilityOverridden', true,
      'supervisorAcknowledgementRequired', true
    )
  );

  -- The established booking trigger calculates the assignment from the new
  -- exact-slot commitment, closes the old assignment and sends notifications.
  ...
  set assignment_reason = case
    when v_previous_supervisor_id is null
      then 'Allocated by a CFI / DCFI despite roster unavailability'
    else 'Reassigned by a CFI / DCFI from the calendar'
  end
  ...
  insert into public.operations_audit_events (
    ...
    v_booking.id,
    case
      when v_previous_supervisor_id is null then 'cfi_supervisor_allocated'
      else 'cfi_supervisor_reassigned'
    end,
    v_actor_id,
    jsonb_build_object(
      'supervisionStatus', v_previous_supervision_status,
      'supervisingInstructorId', v_previous_supervisor_id
    ),
    jsonb_build_object(
...
set rationale = 'CFI/DCFI-only allocation and reassignment action. It locks the booking and enforces target authorisation, scope, qualification, duty and capacity while deliberately bypassing roster availability.',
    reviewed_at = date '2026-09-03'
where signature = 'public.assign_booking_supervisor(p_booking_id uuid, p_supervisor_id uuid)';

select private.assert_function_permission_manifest();

comment on function public.assign_booking_supervisor(uuid, uuid) is
  'Lets a CFI/DCFI allocate or reassign an authorised senior instructor to a future supervised booking without requiring roster availability. A changed supervisor must acknowledge the new assignment.';
