-- Import and export the course-level lesson criteria matrix (for example NC/S/C)
-- alongside the existing detailed numeric syllabus matrix.
  ...
  loop
    ...
    if p_record_type = 'lesson' and v_row ? 'criteria_grades' then
      ...
      loop
        ...
        select lesson.pass_marks->>v_criterion_id
        into v_lesson_pass_mark
        from public.training_lessons lesson
        where lesson.id = v_lesson_id
          and lesson.course_id = p_course_id;

        if v_lesson_pass_mark is null or v_lesson_pass_mark = '-' then
          v_errors := v_errors + 1;
          v_results := v_results || jsonb_build_array(jsonb_build_object(
            'source_row', v_source_row,
            'status', 'error',
            'messages', jsonb_build_array(format('Criterion %s is not configured for this lesson.', v_criterion_id))
          ));
        end if;
      end loop;
    end if;
...
comment on function public.process_student_course_record_import(
  uuid, uuid, text, text, text, jsonb, boolean, boolean
) is 'Validates duplicate references and imports course criteria grades plus detailed syllabus competencies through the atomic course record transfer core.';
