-- Preserve valid historical instructor assessments even when the current
-- lesson pass-mark matrix no longer expects that course criterion. A lesson's
-- matrix defines its target; it is not an allow-list for historical evidence.
...
comment on function public.process_student_course_record_import(
  uuid, uuid, text, text, text, jsonb, boolean, boolean
) is 'Validates and imports course record transfers while preserving valid course-level historical assessment evidence outside the current lesson target matrix.';
