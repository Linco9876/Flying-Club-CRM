...
import { photonAddress } from "../_shared/addressAutocomplete.ts";
...
const searchPhoton = async (query: string): Promise<AddressSuggestion[]> => {
  ...
  return uniqueSuggestions(features.map((feature: Record<string, any>) => ({
    id: `osm:${clean(feature?.properties?.osm_type)}:${
      clean(feature?.properties?.osm_id)
    }`,
...
Deno.serve(async (request) => {
  ...
  if (rateLimitExceeded(request)) {
    return json({
      error: "Too many address searches. Wait a moment and try again.",
    }, 429);
  }
  ...
  try {
    ...
    return json({
      ...
      message:
        "Suggestions are temporarily unavailable. Enter the address manually.",
    });
  }
});
