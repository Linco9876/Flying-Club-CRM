...
const stripeModeFromEvent = (event: any): StripeMode =>
  event?.livemode === true ? "live" : "test";
  ...
  for (const candidate of secrets) {
    ...
    try {
      ...
      if (eventMode !== candidate.mode) {
        throw new Error(
          `Stripe webhook mode mismatch. Event is ${eventMode}, but ${candidate.mode} secret verified it.`,
        );
      }
...
const firstJoinRow = (value: unknown) =>
...
const fetchStripeObject = async ({
  ...
  if (!response.ok) {
    throw new Error(
      body?.error?.message || `Stripe request failed with ${response.status}`,
    );
  }
  return body;
};
...
const waitForXeroApiSlot = async () => {
  ...
  for (let attempt = 1; attempt <= 8; attempt += 1) {
    ...
    if (waitMs > xeroMaxWaitMs) {
      ...
      throw Object.assign(
        new Error(
          `Xero is busy. The CRM is pausing Xero calls and will retry in about ${retryAfterSeconds} seconds.`,
        ),
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const response = await fetch(`https://api.xero.com/api.xro/2.0/${path}`, {
      ...
      headers: {
        ...
        ...(asString(idempotencyKey)
          ? { "Idempotency-Key": asString(idempotencyKey) }
          : {}),
      },
    ...
    if (!response.ok) {
      const message = payload?.Message || payload?.Title || payload?.Detail ||
...
const refreshXeroAccessToken = async (
  adminClient: SupabaseAdminClient,
  connection: any,
) => {
...
const getXeroPaymentContext = async (adminClient: SupabaseAdminClient) => {
  ...
  if (!connection?.tenant_id || connection?.disconnected_at) {
    throw new Error("Xero is not connected.");
  }
  const accountCode = clean(settings?.stripe_payment_account_code);
  if (!accountCode) {
    throw new Error(
      "Set the Xero Stripe clearing account before taking invoice payments.",
    );
  }
  return {
...
const getFlightLogForSession = async (
  adminClient: SupabaseAdminClient,
  session: any,
) => {
  const metadataFlightLogId = asString(
    session.metadata?.flight_log_id || session.client_reference_id,
  );
...
const getVoucherForSession = async (
  adminClient: SupabaseAdminClient,
  session: any,
) => {
  const metadataVoucherId = asString(
    session.metadata?.voucher_id || session.client_reference_id,
  );
  ...
  if (!response.ok) {
    throw new Error(
      typeof body?.error === "string"
        ? body.error
        : `Xero top-up sync failed with HTTP ${response.status}`,
    );
  }
...
const getVerifiedFlightPaymentTotal = async (
  adminClient: SupabaseAdminClient,
  flightLogId: string,
) => {
  const { data, error } = await adminClient
...
const getApplicableFlightPaymentAmount = ({
  ...
  const appliedAmount = Math.max(
    0,
    Math.min(roundCurrency(receivedAmount), remaining),
  );
  return {
    appliedAmount,
...
const syncFlightLogFromXeroInvoicePayment = async ({
  ...
  const alreadyRecorded = await hasFlightPaymentReference(
    adminClient,
    flightLog.id,
    paymentReference,
  );
  ...
  if (!alreadyRecorded && amountApplied > 0) {
    const aircraft = firstJoinRow(flightLog.aircraft) as {
    ...
    const { error: transactionError } = await adminClient
      ...
      .insert({
        ...
        description:
          `Stripe invoice payment (${paymentReference}) - ${registration} flight on ${flightDate}`,
...
const handleMemberTopupStripeEvent = async ({
  ...
  if (!userId) {
    throw new Error("Member top-up checkout is missing the user id.");
  }

  const paymentMethod = await getStripePaymentMethod(adminClient, session);
  const amount = stripeAmountToDollars(session.amount_total) ??
    Number(session.metadata?.topup_amount || 0);
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new Error("Member top-up checkout has no valid amount.");
  }
  ...
  if (existing) {
    ...
    if (!asString(existing.xero_bank_transaction_id)) {
      await queueMemberTopupXeroSync(
        adminClient,
        existing.id,
        "stripe_topup_paid_duplicate_event",
      );
      try {
        syncResult = await processMemberTopupXeroSync({
          supabaseUrl,
          serviceRoleKey,
...
const handleXeroInvoicePaymentStripeEvent = async ({
  ...
  const paymentRecordId = asString(
    session.metadata?.payment_record_id || session.client_reference_id,
  );
  const invoiceId = asString(session.metadata?.xero_invoice_id);
  if (!paymentRecordId || !invoiceId) {
    throw new Error("Xero invoice payment checkout is missing CRM metadata.");
  }
  ...
  const invoice = Array.isArray(invoiceResult?.Invoices)
    ? invoiceResult.Invoices[0]
    : null;
  if (!invoice) throw new Error("Xero invoice not found for Stripe payment.");
  const invoiceContactId = asString(
    invoice?.Contact?.ContactID || invoice?.ContactID,
  );
  if (invoiceContactId !== asString(record.xero_contact_id)) {
    throw new Error(
      "Xero invoice contact no longer matches the CRM payment record.",
    );
  }

  const amountPaidToStripe = stripeAmountToDollars(session.amount_total) ??
  ...
  if (amountToApply <= 0) {
    await adminClient.from("xero_invoice_portal_payments").update({
      ...
      stripe_payment_intent_id: asString(session.payment_intent) || null,
      error:
        "Stripe payment succeeded but the Xero invoice has no amount due. Review refund or allocation manually.",
      updated_at: new Date().toISOString(),
    }).eq("id", record.id);
  ...
  await adminClient.from("xero_invoice_portal_payments").update({
    status: money(amountPaidToStripe) > amountToApply + 0.005
      ? "needs_review"
      : "paid",
    ...
    error: money(amountPaidToStripe) > amountToApply + 0.005
      ? `Stripe collected ${money(amountPaidToStripe).toFixed(2)} but only ${
        amountToApply.toFixed(2)
      } was applied to Xero. Review the difference.`
      : null,
    updated_at: new Date().toISOString(),
  }).eq("id", record.id);
...
const handleMemberCardSetupEvent = async ({
  ...
  if (!setupRecordId || !userId) {
    throw new Error("Stripe card setup session is missing CRM metadata");
  }
  ...
  if (!setupIntentId) {
    throw new Error("Stripe Checkout did not include a SetupIntent");
  }
  ...
  if (!paymentMethodId) {
    throw new Error("Stripe SetupIntent did not include a payment method");
  }
  ...
  const { data: setupRecord, error: setupError } = await adminClient
    .from("member_stripe_card_setup_sessions")
    .select(
      "id,user_id,stripe_customer_id,consent_text,consent_accepted_at,consent_ip,consent_user_agent",
    )
    .eq("id", setupRecordId)
    .eq("user_id", userId)
    .maybeSingle();
  ...
  if (!setupRecord) {
    throw new Error("CRM card setup consent record was not found");
  }
    ...
    .upsert({
      ...
      stripe_customer_id: asString(
        setupIntent.customer || session.customer ||
          setupRecord.stripe_customer_id,
      ),
...
const handleMembershipPaymentSetupEvent = async ({
  adminClient,
  stripeSecretKey,
  event,
  session,
  ...
  if (!setupRecordId || !userId) {
    throw new Error("Membership payment setup is missing CRM metadata");
  }
  ...
  if (event.type === "checkout.session.expired") {
    await adminClient.from("membership_payment_setup_sessions").update({
      status: "expired",
      updated_at: new Date().toISOString(),
    }).eq("id", setupRecordId).eq("user_id", userId);
    await adminClient.from("membership_payment_preferences").update({
      authority_status: "failed",
      last_collection_error: "Payment setup expired before completion.",
      updated_at: new Date().toISOString(),
    }).eq("user_id", userId).eq("authority_status", "pending");
    return json({ received: true, membershipPaymentSetup: "expired" });
  }
  ...
  if (setupError) throw setupError;
  if (!setupRecord) {
    throw new Error("Membership payment consent record was not found");
  }
  if (setupRecord.status === "cancelled") {
    return json({
      received: true,
      membershipPaymentSetup: "ignored_after_membership_cancellation",
    });
  }

  const paymentType = asString(paymentMethod.type) ||
    (setupRecord.payment_method === "becs" ? "au_becs_debit" : "card");
  const methodKind = paymentType === "au_becs_debit" ? "becs" : "card";
  if (methodKind !== setupRecord.payment_method) {
    throw new Error(
      "Stripe returned a different payment method than the member selected",
    );
  }
  const details = methodKind === "becs"
    ? paymentMethod.au_becs_debit || {}
    : paymentMethod.card || {};
  const last4 = asString(details.last4);
  const display = methodKind === "becs"
    ? `Bank account ending ${last4 || "----"}`
    : `${asString(details.brand) || "Card"} ending ${last4 || "----"}`;
  const stripeMode = stripeModeFromEvent(event);
  const now = new Date().toISOString();
  ...
  }).eq("user_id", userId).eq("payment_method", methodKind);
  if (preferenceError) throw preferenceError;

  if (methodKind === "card") {
    await adminClient.from("member_stripe_payment_methods").update({
      active: false,
      is_default: false,
      updated_at: now,
    }).eq("user_id", userId).eq("active", true);
    const { error: cardError } = await adminClient.from(
      ...
      stripe_payment_method_id: paymentMethodId,
      stripe_setup_intent_id: setupIntentId,
      card_brand: asString(details.brand) || null,
      card_last4: last4 || null,
      card_exp_month: Number(details.exp_month) || null,
      card_exp_year: Number(details.exp_year) || null,
      active: true,
      is_default: true,
      consent_text: setupRecord.consent_text,
      consent_accepted_at: setupRecord.consent_accepted_at,
  ...
  await adminClient.from("membership_payment_setup_sessions").update({
    stripe_checkout_session_id: asString(session.id) || null,
    stripe_setup_intent_id: setupIntentId,
    status: "completed",
    updated_at: now,
    ...stripeModeColumns(stripeMode),
  ...
  try {
    const response = await fetch(`${supabaseUrl}/functions/v1/xero-sync`, {
      ...
      body: JSON.stringify({
        action: "issue-member-membership-invoice",
        userId,
        sendEmail: true,
      }),
    });
    billing = await response.json().catch(() => ({}));
    ...
    billingError = error instanceof Error
      ? error.message
      : "Membership invoice could not be issued yet.";
    console.warn(
      "Membership payment setup completed; invoice issue deferred:",
      billingError,
    );
  }
  ...
  return json({
    received: true,
    membershipPaymentSetup: "completed",
    paymentMethod: methodKind,
    billing,
    billingError,
  });
};

const handleMembershipInvoicePaymentIntentEvent = async ({
  adminClient,
  event,
  paymentIntent,
}: {
  ...
  const invoiceId = asString(paymentIntent.metadata?.xero_invoice_id);
  if (!paymentRecordId || !periodId || !userId || !invoiceId) {
    throw new Error("Membership payment is missing CRM metadata");
  }
  const { data: record, error: recordError } = await adminClient
    .from("xero_invoice_portal_payments")
    .select("*")
    ...
    .maybeSingle();
  if (recordError) throw recordError;
  if (!record) throw new Error("Membership payment record was not found");
  const now = new Date().toISOString();
  ...
  const invoice = Array.isArray(invoiceResult?.Invoices)
    ? invoiceResult.Invoices[0]
    : null;
  if (!invoice) throw new Error("Xero membership invoice was not found");
  const invoiceContactId = asString(
    invoice?.Contact?.ContactID || invoice?.ContactID,
  );
  if (invoiceContactId !== asString(record.xero_contact_id)) {
    throw new Error(
      "Xero membership invoice contact no longer matches the payment authority",
    );
  }
  ...
  const amountDue = money(invoice.AmountDue);
  const amountToApply = Math.min(amountReceived, amountDue);
  ...
  const paymentResult = await xeroRequest({
    ...
    tenantId: ctx.connection.tenant_id,
    accessToken: ctx.connection.access_token,
    idempotencyKey: `membership-payment-${asString(paymentIntent.id)}`,
    body: {
      Payments: [{
        Invoice: { InvoiceID: invoiceId },
        Account: { Code: ctx.stripeClearingAccountCode },
        Date: new Date().toISOString().slice(0, 10),
        Amount: amountToApply,
        Reference: `Stripe membership payment ${asString(paymentIntent.id)}`,
      }],
    },
  });
  const xeroPaymentId = asString(paymentResult?.Payments?.[0]?.PaymentID);
  if (!xeroPaymentId) {
    throw new Error("Xero did not return a membership payment ID");
  }

  await adminClient.from("xero_invoice_portal_payments").update({
    status: amountReceived > amountToApply + 0.005 ? "needs_review" : "paid",
    stripe_payment_intent_id: asString(paymentIntent.id),
    xero_payment_id: xeroPaymentId,
    amount: amountToApply,
    error: amountReceived > amountToApply + 0.005
  ...
  await adminClient.from("membership_financial_periods").update({
    fee_disposition: amountDue - amountToApply <= 0.005 ? "paid" : "invoiced",
    financially_cleared_at: amountDue - amountToApply <= 0.005 ? now : null,
    xero_invoice_status: amountDue - amountToApply <= 0.005
      ? "PAID"
  ...
  await adminClient.from("notifications").insert({
    ...
    message:
      "Your membership payment has been received and reconciled to Xero.",
    metadata: {
...
const handleFlightLogPaymentIntentEvent = async ({
  ...
  if (flightError) throw flightError;
  if (!flightLog) {
    throw new Error("Flight log not found for Stripe PaymentIntent");
  }

  const paymentMethod = await getStripePaymentMethod(adminClient, {
    metadata: paymentIntent.metadata || {},
  });
  const now = new Date().toISOString();
  const amount = stripeAmountToDollars(
    paymentIntent.amount_received || paymentIntent.amount,
  ) ?? Number(flightLog.calculated_cost || 0);
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    const flightDate = flightLog.start_time
      ...
      : "flight";
    const description =
      `Stripe saved card payment (${paymentIntent.id}) - ${aircraftRegistration} flight on ${flightDate}`;
    const alreadyRecorded = await hasFlightPaymentReference(
      adminClient,
      flightLog.id,
      paymentIntent.id,
    );
    const paidBefore = await getVerifiedFlightPaymentTotal(
      adminClient,
      flightLog.id,
    );
...
const handleFlightLogStripeEvent = async ({
  ...
  try {
    ...
    const eventState = await recordFlightStripeEvent({
      adminClient,
      event,
      flightLogId,
      sessionId,
    ...
    if (!flightLog) {
      throw new Error("No flight log matched this Stripe Checkout Session");
    }

    const stripePaymentIntentId = asString(session.payment_intent) || null;
    const updateBase = {
      stripe_checkout_session_id: sessionId ||
        flightLog.stripe_checkout_session_id,
    ...
    const paymentMethod = await getStripePaymentMethod(adminClient, session);
    const paidAmount = stripeAmountToDollars(session.amount_total) ??
      Number(flightLog.calculated_cost || 0);
    const flightDate = flightLog.start_time
      ? new Date(flightLog.start_time).toLocaleDateString("en-AU")
      : "flight";
    const aircraftRegistration = asString(flightLog.aircraft?.registration) ||
      "aircraft";
    const description =
      `Stripe card payment (${sessionId}) - ${aircraftRegistration} flight on ${flightDate}`;
    const alreadyRecorded = await hasFlightPaymentReference(
      adminClient,
      flightLog.id,
      sessionId,
    ...
    const message = error instanceof Error
      ? error.message
      : "Unexpected Stripe flight payment webhook error";
    console.error("flight payment webhook error:", error);
    if (event?.id) {
      await markFlightEventProcessed(adminClient, event.id, message);
    }
...
const sendVoucherEmail = async ({
  ...
  const response = await fetch(
    `${supabaseUrl}/functions/v1/send-trial-voucher-email`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        voucherId,
        redirectOrigin: Deno.env.get("PUBLIC_SITE_URL") ||
  ...
  if (!response.ok) {
    throw new Error(
      body?.error || `Voucher email failed with ${response.status}`,
    );
  }
...
const sendBookNowConfirmationEmail = async ({
  ...
  const bookingUrl = `${siteOrigin()}/trial-flight-voucher?voucherCode=${
    encodeURIComponent(voucher.code)
  }`;
...
<html>
  <body style="margin:0;background:#eef4fb;font-family:Arial,Helvetica,sans-serif;color:#0f172a;">
    ...
    escapeHtml(intro)
            ...
            <tr>
              <td style="padding:30px;">
                <p style="margin:0 0 16px;font-size:16px;line-height:1.65;">Hi ${
    escapeHtml(toName || "there")
  },</p>
                <p style="margin:0 0 22px;color:#334155;font-size:16px;line-height:1.65;">${
    escapeHtml(product?.name || "Trial instructional flight")
  }</p>
                <div style="border:1px solid #dbeafe;background:#f8fbff;border-radius:18px;padding:18px;margin-bottom:18px;">
                  ...
                  <p style="margin:0;font-size:20px;line-height:1.35;color:#0f172a;font-weight:800;">${
    escapeHtml(dateLabel)
                ...
                <div style="border:1px solid #dbeafe;background:#f8fbff;border-radius:18px;padding:18px;margin-bottom:22px;">
                  ...
                  <p style="margin:0;font-size:20px;line-height:1.35;color:#0f172a;font-weight:800;">${
    escapeHtml(timeFormatter.format(start))
  } - ${escapeHtml(timeFormatter.format(end))}</p>
  ...
  }" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;font-weight:800;border-radius:14px;padding:14px 22px;">View or reschedule booking</a>
                </p>
                <p style="margin:0;color:#64748b;font-size:13px;line-height:1.65;">Voucher code: <strong style="color:#0f172a;">${
  ...
  if (!response.ok) {
    ...
    return {
      sent: false,
      error: body || `Brevo email failed with ${response.status}`,
    };
  }
...
const sendBookNowAbandonedEmail = async ({ voucher }: { voucher: any }) => {
  ...
  if (voucher.checkout_abandoned_email_sent_at) {
  ...
  const senderEmail = Deno.env.get("BREVO_SENDER_EMAIL") ||
    "no-reply@bendigoflyingclub.com.au";
  const senderName = Deno.env.get("BREVO_SENDER_NAME") || "Bendigo Flying Club";
  const chooseNewTimeUrl =
    `${siteOrigin()}/trial-flight-gift-vouchers?checkout=cancelled`;
...
<html>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border-radius:22px;overflow:hidden;box-shadow:0 16px 40px rgba(15,23,42,.12);">
            ...
            <tr>
              <td style="padding:26px;">
                <p style="margin:0 0 16px;font-size:16px;line-height:1.6;">Hi ${
    escapeHtml(voucher.purchaser_name || "there")
  },</p>
  ...
  }" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;font-weight:800;border-radius:14px;padding:14px 22px;">Continue checkout</a>
                </p>
                <p style="margin:0;">
                  <a href="${
    escapeHtml(chooseNewTimeUrl)
  }" style="color:#2563eb;font-weight:700;">Choose a different time</a>
                </p>
              </td>
            </tr>
          </table>
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      subject: testModeSubject(
        voucher?.stripe_mode === "test" || voucher?.is_test_mode === true
          ? "test"
          : "live",
...
const ensureBookNowVoucherAccount = async (
  adminClient: SupabaseAdminClient,
  voucher: any,
) => {
  const fullName = clean(
    voucher.send_to_recipient ? voucher.recipient_name : voucher.purchaser_name,
  ) || clean(voucher.purchaser_name) || "Trial flight guest";
  const email = clean(
    voucher.send_to_recipient
      ? voucher.recipient_email
      : voucher.purchaser_email,
  ).trim().toLowerCase() ||
    clean(voucher.purchaser_email).trim().toLowerCase();
  ...
  if (
    existingProfile && existingProfile.portal_access_scope !== "trial_voucher"
  ) {
    throw new Error(
      "The booking email already belongs to a full CRM account. Staff need to link this voucher manually.",
    );
  }
  ...
  if (!userId) {
    ...
    if (authError || !authData.user) {
      throw new Error(
        authError?.message || "Failed to create voucher booking account",
      );
    }
  ...
  await adminClient.from("students").upsert({ id: userId }, {
    onConflict: "id",
  });
  return userId;
};

const handleBookNowPaidVoucher = async (
  adminClient: SupabaseAdminClient,
  voucher: any,
) => {
  ...
  if (
    !voucher.held_aircraft_id || !voucher.held_instructor_id ||
    !voucher.held_start_time || !voucher.held_end_time
  ) {
  ...
  const { data: bookingId, error: bookingError } = await adminClient.rpc(
    "book_trial_flight_voucher_slot",
    {
      ...
      p_notes: `Trial flight voucher ${voucher.code} - ${
        product.name || "Trial flight"
      }`,
    },
  );
  ...
  if (bookingError || !bookingId) {
    throw new Error(
      bookingError?.message || "Could not create the paid trial flight booking",
    );
  }
  ...
  const shouldEmailRecipient = Boolean(
    voucher.send_to_recipient && recipientEmail &&
  ...
  await adminClient
    .from("trial_flight_vouchers")
    .update({
      purchaser_confirmation_sent_at: purchaserResult.sent
        ? new Date().toISOString()
        : null,
      purchaser_confirmation_error: purchaserResult.error || null,
      recipient_confirmation_sent_at: recipientResult.sent
        ? new Date().toISOString()
        : null,
  ...
  return {
    bookingId,
    purchaserEmail: purchaserResult,
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !internalSecret) {
    return json(
      { error: "Stripe voucher webhook is not fully configured" },
      503,
    );
  }
  ...
  try {
    ...
    return json({
      error: error instanceof Error
        ? error.message
        : "Stripe API key is not configured for this webhook mode.",
    }, 503);
  }
  ...
  try {
    ...
    if (
      ...
      if (
        asString(session.metadata?.crm_payment_type) ===
          "flight_log_off_session"
      ) {
        return await handleFlightLogPaymentIntentEvent({
          adminClient,
          event,
          paymentIntent: session,
        });
    ...
    if (
      ...
      return await handleMembershipPaymentSetupEvent({
        adminClient,
        stripeSecretKey,
        event,
        session,
    ...
    if (asString(session.metadata?.crm_payment_type) === "member_card_setup") {
      return await handleMemberCardSetupEvent({
        adminClient,
        stripeSecretKey,
        event,
        session,
    ...
    if (asString(session.metadata?.crm_payment_type) === "member_topup") {
      return await handleMemberTopupStripeEvent({
        adminClient,
        event,
        session,
        sessionId,
    ...
    if (
      asString(session.metadata?.crm_payment_type) === "xero_invoice_payment"
    ) {
      return await handleXeroInvoicePaymentStripeEvent({
        adminClient,
        event,
        session,
        sessionId,
    ...
    if (
      asString(session.metadata?.crm_payment_type) === "flight_log" ||
      asString(session.metadata?.flight_log_id)
    ) {
      return await handleFlightLogStripeEvent({
        adminClient,
        event,
        session,
        sessionId,
    ...
    if (!voucher) {
      throw new Error(
        "No trial flight voucher matched this Stripe Checkout Session",
      );
    }
    ...
    if (event.type === "checkout.session.expired") {
      ...
      await adminClient
        ...
        .update({
          ...
          stripe_checkout_session_id: sessionId ||
            voucher.stripe_checkout_session_id,
          stripe_payment_intent_id: asString(session.payment_intent) || null,
          checkout_abandoned_email_sent_at: abandonedEmail.sent
            ? new Date().toISOString()
            : voucher.checkout_abandoned_email_sent_at || null,
