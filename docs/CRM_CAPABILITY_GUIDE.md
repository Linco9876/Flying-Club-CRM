...
- Duty start, finish and break fields use a touch-friendly clock-face picker with high-contrast light and dark themes, large hour/minute controls, explicit AM/PM selection, exact-minute dial input, date selection and a typed-time accessibility fallback. Hovering over the hour or minute readout and using the scroll wheel adjusts that value; arrow keys provide the same fine control. The Android Duty Clock app explicitly requests the matching native clock presentation.
